...
def brickwall_gate_custom(t, qubit_i, qubit_j, num_qubits, terms_on_pair):
    """Construct exp(-i t H_ij) where H_ij = sum c * P_i ⊗ P_j for qubit pair (i, j)."""
    H_ij = jnp.zeros((4, 4), dtype=jnp.complex128)

    for coeff, pauli_string in terms_on_pair:
        # Always assume pauli_string contains exactly two qubits: i and j
        pauli_i = PAULI_MAP[pauli_string[qubit_i]]
        pauli_j = PAULI_MAP[pauli_string[qubit_j]]
        H_ij += coeff * jnp.kron(pauli_i, pauli_j)

    return expm(-1j * t * H_ij)


def get_brickwall_trotter_gates_custom(
    t, n_sites, hamiltonian_terms, n_repetitions=1, degree=2, use_TN=False
):
    if isinstance(hamiltonian_terms, list):
        term_dict = {}
        for i, j, P1, P2, coeff in hamiltonian_terms:
            key = tuple(sorted((i, j)))
            pauli_dict = {i: P1, j: P2}
            if key not in term_dict:
                term_dict[key] = []
            term_dict[key].append((coeff, pauli_dict))
        hamiltonian_terms = term_dict

    dt = t / n_repetitions
    padded_sites = n_sites + (n_sites % 2)
    dummy_site = padded_sites - 1 if n_sites % 2 else None

    odd_pairs = [(i, i + 1) for i in range(0, padded_sites - 1, 2)]
    even_pairs = [(i, i + 1) for i in range(1, padded_sites - 1, 2)]

    def make_layer(pairs):
        gates = []
        for i, j in pairs:
            if dummy_site is not None and (i == dummy_site or j == dummy_site):
                continue
            # extract all terms for this pair
            terms_ij = hamiltonian_terms.get((i, j), []) + hamiltonian_terms.get((j, i), [])
            g = brickwall_gate_custom(dt / degree, i, j, padded_sites, terms_ij)
            gates.append(g)
        return gates

    if degree in [1, 2]:
        L1 = make_layer(odd_pairs)
        L2 = make_layer(even_pairs)
        ...
        if use_TN and gates.shape[1:] == (4, 4):
            gates = gates.reshape((len(gates), 2, 2, 2, 2))
        return gates

    else:
        raise NotImplementedError("Only degree=1 or 2 supported for custom Hamiltonians.")
