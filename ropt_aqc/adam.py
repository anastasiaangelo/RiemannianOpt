...
class RieADAM():
    ...
    def minimize(
        ...
        while self._t < self._maxiter:
            ...
            # standard exponential exponential via correction of ADAM
            lr_eff = self._lr * jnp.sqrt(1 - self._beta_2 ** self._t) / (1 - self._beta_1 ** self._t) 
            # Decaying learning rate
            # lr_eff = (self._lr / (1 + 0.0005 * self._t)) * jnp.sqrt(1 - self._beta_2 ** self._t) / (1 - self._beta_1 ** self._t)
            
            # adaptive learning rate based on gradient norm
            # grad_norm = jnp.linalg.norm(derivative)
            # decay_factor = 1.0 / (1 + 20.0 * grad_norm)  # tune 20.0 if needed
            # lr_eff = (self._lr * decay_factor) * jnp.sqrt(1 - self._beta_2 ** self._t) / (1 - self._beta_1 ** self._t)

            new_step = - lr_eff * self._m / (jnp.sqrt(self._v) + self._noise_factor)[:,jnp.newaxis,jnp.newaxis]
            # # Gradient norm clipping
            # step_norm = jnp.linalg.norm(new_step)
            # new_step = jnp.where(step_norm > 1.0, new_step * (1.0 / step_norm), new_step)
        ...
        return params_best, self._t, loss1
