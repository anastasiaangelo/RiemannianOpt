...
def load_reference(path, N, n_repetitions, t):
    fname = str(N) + '_reference_' + str(n_repetitions) + '_' + str(t) + '.pkl'
    ...
    if 'fermi-hubbard-1d' in path:
        ...
        return U_mpo, t, n_sites, degree, n_repetitions, err_threshold, hamiltonian, H, ref_seed
    elif 'pxp' in path:
        return U_mpo, t, n_sites, degree, n_repetitions, err_threshold, hamiltonian, H, ref_seed
...
def save_reference(path, U_mpo, t, n_sites, degree, n_repetitions, 
    ...
    if type(ref_nbr) is not type(None):
        # fname = os.path.join(path, str(N) + '_reference_' + str(n_repetitions) +'_'+ str(t) +'_'+ str(ref_nbr) + '.pkl')
        fname = os.path.join(path, str(N) + '_reference_' + str(n_repetitions) +'_'+ str(t) + '.pkl')
    else:
        fname = os.path.join(path, str(N)+'_reference'+'.pkl')  
    print('saved to path:' , fname)        
