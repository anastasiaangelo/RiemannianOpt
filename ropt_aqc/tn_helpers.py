...
def left_to_right_SVD_sweep(mpo_res, max_bondim=128):
    ...
    if s.size == 0:
        print(f"⚠️ Empty singular values encountered at site {i}, skipping normalization.")
        mpo_res[-1] = U  # or consider using an identity tensor or fallback
        return mpo_res
...
def canonicalize_local_tensor(T, left=True):
    ...
    if R.ndim == 3 and R.shape[1] == 1:
        R = R.squeeze(1)            # (χ,1,χ̃) → (χ,χ̃)
    elif R.ndim == 3 and R.shape[0] == 1:
        R = R.squeeze(0)            # (1,χ,χ̃) → (χ,χ̃)
...
def compute_overlap_mpo(mpo1, mpo2):
    """
    Computes ⟨mpo1 | mpo2⟩ by converting both MPOs to MPS,
    canonically normalizing them, and taking inner product.
    """
    from ropt_aqc.tn_helpers import convert_mpo_to_mps, get_left_canonical_mps, inner_product_mps

    mps1 = convert_mpo_to_mps(mpo1)
    mps2 = convert_mpo_to_mps(mpo2)

    mps1, _ = get_left_canonical_mps(mps1, normalize=False, get_norm=True)
    mps2, _ = get_left_canonical_mps(mps2, normalize=False, get_norm=True)

    return inner_product_mps(mps1, mps2)


def mpo_to_dense(mpo):
    """Convert an MPO (list of tensors) to a dense matrix"""
    from functools import reduce
    import jax.numpy as jnp

    # mpo[i] has shape (left_bond, physical_in, physical_out, right_bond)
    n_sites = len(mpo)
    d_phys = mpo[0].shape[1]

    # Start from first tensor
    current = mpo[0]  # shape: (1, d, d, D)
    current = jnp.transpose(current, (0, 1, 2, 3))  # Just to be clear

    for i in range(1, n_sites):
        A = current  # shape: (l1, d1, d1, r1)
        B = mpo[i]   # shape: (l2=r1, d2, d2, r2)

        # Contract right bond of A with left bond of B
        C = jnp.tensordot(A, B, axes=[-1, 0])  # shape: (l1, d1, d1, d2, d2, r2)

        # Merge physical legs
        C = jnp.transpose(C, (0, 1, 3, 2, 4, 5))
        C = C.reshape(C.shape[0], d_phys**(i+1), d_phys**(i+1), C.shape[-1])

        current = C

    # Now contract final edge dimension (left bond 1, right bond 1)
    final = current.squeeze()  # shape: (2^n, 2^n)
    return final